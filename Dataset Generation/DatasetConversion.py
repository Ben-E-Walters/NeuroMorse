...
file = h5py.File('NeuroMorse_Train.h5','w')
file_test = h5py.File('NeuroMorse_Test.h5','w')
...
for d in Dropout:
    for j in Jitter:
        for p in Poisson:
            ...
            Spike_group = file.create_group('%s/Spikes' %(name))
            dt = h5py.vlen_dtype(np.dtype('int32'))
            dt_labels = h5py.string_dtype()
            ...
            Label_group = file.create_group('%s/Labels'%(name))
            ...
            Spike_group = file_test.create_group('%s/Spikes' %(name))
